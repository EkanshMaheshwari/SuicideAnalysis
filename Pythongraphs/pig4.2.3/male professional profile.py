...
data=data[data[1]=='Male']
data["numerical"]=z.fit_transform(data[0])
k=data[["numerical",2]]
kmeans.fit(k)
color=np.array(["Green","Red","Brown"])
plt.scatter(data[0],data[2],c=color[kmeans.labels_])
plt.xlabel("Profesional profile of male commiting suicide")
plt.ylabel("Number of Suicide")
plt.title("Clustering by the professional profile of the male commiting suicide")
